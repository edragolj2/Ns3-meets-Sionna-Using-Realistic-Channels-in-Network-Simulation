...
data = load('txid1_coordinates_emms_speed12.txt');  % Pretpostavlja se da se datoteka nalazi u trenutnom direktoriju
...
xlim([-40 80]);   
ylim([0 170]); 
title('Kretanje tacke (v=12 m/s)');
grid on;
axis equal;  % Da se o?uva razmjera izme?u osi

% U?itaj podatke iz .txt fajla
data = load('timestamp_rxpower_emms_speed12.txt');
...
xlabel('Vrijeme [s]');
ylabel('SNR [dB]');
title('SNR u funkciji vremena (v=12 m/s)');
